...
class KeyTest < ActiveSupport::TestCase
  setup do
    ...
    @key = HealthCards::KeyPair.new(@key_path)
  end
  ...
  test 'Use existing keys if they exist' do
    ...
    new_jwks = HealthCards::KeyPair.new(@key_path).public_key.to_json

    assert_equal original_jwks, new_jwks
  end

  test 'create a signed jws' do
    card = HealthCards::Card.new(private_key: @key.private_key, public_key: @key.public_key, payload: 'asdfasdf')
